...
class CreditsEditor extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if(!quitting && !blockInput)
		{
			...
			{
				var shiftMult:Int = 1;
				if (FlxG.keys.justPressed.W || FlxG.keys.justPressed.UP #if android || _virtualpad.buttonUp.justPressed #end)
				{
					changeSelection(-shiftMult);
					holdTime = 0;
				}
				if (FlxG.keys.justPressed.S || FlxG.keys.justPressed.DOWN #if android || _virtualpad.buttonDown.justPressed #end)
				{
					changeSelection(shiftMult);
					holdTime = 0;
				}
				...
				if((FlxG.keys.justPressed.S || FlxG.keys.justPressed.DOWN) || (FlxG.keys.justPressed.W || FlxG.keys.justPressed.UP) #if android || (_virtualpad.buttonUp.justPressed || _virtualpad.buttonDown.justPressed) #end)
				{
			...
			if(FlxG.keys.justPressed.DELETE #if android || _virtualpad.buttonY.justPressed #end) {
				deleteSelItem();
			}
			...
			if(FlxG.keys.justPressed.ONE #if android || _virtualpad.buttonZ.justPressed #end) {
				addTitle();
			}
			if(FlxG.keys.justPressed.TWO #if android || _virtualpad.buttonC.justPressed #end) {
				addCredit();
			}

			if (FlxG.keys.justPressed.BACKSPACE || FlxG.keys.justPressed.ESCAPE #if android || _virtualpad.buttonB.justPressed #end)
			{
				if(colorTween != null) {
					colorTween.cancel();
				}
	...
	function saveCredits() {
		...
		#if android
			SUtil.saveContent("credits", ".txt", data, 'credits');
		#else
		if (data.length > 0)
		{
